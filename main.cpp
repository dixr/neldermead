                  ...
                  size_t& idx_min, size_t& idx_max, size_t& idx_second_max, double& delta, vector<double>& centroid)
{
    ...
    delta = 2*fabs(fsimplex[idx_min] - fsimplex[idx_max])/(fabs(fsimplex[idx_min]) + fabs(fsimplex[idx_max]) + eps); // # sign. digits apart (from NR)
    ...
    try
    {
        ...
        if(statefilein >> part) // state file exists
        {
            if(part != "EVALUATIONS" || !(statefilein >> evaluations))
                cerr << "error: state file '" << statefile_name << "' is corrupt (part: EVALUATIONS)" << endl;
            ++evaluations;
            statefilein >> part;
            if(part != "SIMPLEX")
                cerr << "error: state file '" << statefile_name << "' is corrupt (part: SIMPLEX)" << endl;
            for(double& d : simplex)
                statefilein >> d;
            statefilein >> part;
            if(part != "FSIMPLEX")
                cerr << "error: state file '" << statefile_name << "' is corrupt (part: FSIMPLEX)" << endl;
            for(double& d : fsimplex)
                statefilein >> d;
            statefilein >> part;
            if(part != "X")
                cerr << "error: state file '" << statefile_name << "' is corrupt (part: X)" << endl;
            for(double& d : x)
                statefilein >> d;
            statefilein >> part;
            if(part != "FX")
                cerr << "error: state file '" << statefile_name << "' is corrupt (part: FX)" << endl;
            for(double& d : fx)
                statefilein >> d;
            statefilein >> mode;
